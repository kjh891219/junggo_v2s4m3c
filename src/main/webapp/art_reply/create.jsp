...
<script>
     function create_login() {
       ...
       <%session.setAttribute("url", "art/read.do?ano="+ano);%>
       return false;
       <% } else { %>
       return true;
       <% } %>
...
<FORM name='frm' method='POST' action='${pageContext.request.contextPath}/art_reply/create.do' class='form-inline'
          onsubmit = 'return create_login()'>
  <input type="hidden" name="nickname" id="nickname" value='${nickname}'>
  <input type="hidden" name="ano" id="ano" value='<%=request.getParameter("ano")%>'>
     ...
     <div style="display: block; position: relative; clear: both;">
        <div style="margin-left: 0; position: relative; margin: 0 10px; display: block;">
          <div style="display: block;">
            <textarea style="width:90%; float:left;" rows="3" name="rcomment" id="rcomment" placeholder="내용을 입력하세요" style="display: block; height: 46px; padding: 4px 8px; border: 1px solid #ddd; border-radius:2px; background: #fff;"><%=request.getParameter("ano")%></textarea>
          </div>
