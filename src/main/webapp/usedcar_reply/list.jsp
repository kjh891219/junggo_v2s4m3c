...
function create(rno){
  ...
  var tag = 
   ...
   "<FORM name='frm' id='frm' method='POST' action='${pageContext.request.contextPath}/usedcar_reply/reply.do' class='form-inline' onsubmit = 'return create_login()'>" +
   "<input type='hidden' name='nickname' id='nickname' value='<c:out value="${nickname}"/>'>" +
   "<input type='hidden' name='u_no' id='u_no' value='<c:out value="${u_no}"/>'>" +
...
<!-- <DIV id='panel_frm' class='content' style='padding: 10px 0px 10px 0px; width: 100%; text-align: center;'>
              <FORM name='frm' id='frm' method='POST'>
                ...
                <input type="hidden" name="u_no" id="u_no" value='1'>
...
<jsp:include page="/usedcar_reply/create.jsp?u_no=${u_no}" flush='false'/>
          ...
          <tr>
            <td class="td_l" style='border: none;'>
             ...
             <c:if test="${(vo.userid eq userid)}">
              <button type='button' onclick="delete_form('${vo.rno}');">${vo.rno }삭제</button>
              </c:if>
              <hr>
