...
CREATE TABLE Mobile(
    ...
    telecome                          VARCHAR2(30)     NULL ,
    userid                            VARCHAR2(20)     NULL,
    FOREIGN KEY (userid) REFERENCES member_test (userid)
);

COMMENT ON TABLE Mobile is '모바일 ';
COMMENT ON COLUMN Mobile.mno is '글 일련 번호';
COMMENT ON COLUMN Mobile.category is '카테고리';
COMMENT ON COLUMN Mobile.nickname is '닉네임';
COMMENT ON COLUMN Mobile.passwd is '패스워드';
COMMENT ON COLUMN Mobile.deal_way is '거래방식';
COMMENT ON COLUMN Mobile.deal_code is '거래구분';
COMMENT ON COLUMN Mobile.product_code is '상품구분';
COMMENT ON COLUMN Mobile.hprice is '희망가격';
COMMENT ON COLUMN Mobile.region is '지역';
COMMENT ON COLUMN Mobile.tel is '전화번호';
COMMENT ON COLUMN Mobile.email is '이메일';
COMMENT ON COLUMN Mobile.quantity is '수량';
COMMENT ON COLUMN Mobile.title is '제목';
COMMENT ON COLUMN Mobile.content is '상세설명';
COMMENT ON COLUMN Mobile.purc_date is '구입시기';
COMMENT ON COLUMN Mobile.wdate is '등록일';
COMMENT ON COLUMN Mobile.cnt is '조회수, 기본값 사용';
COMMENT ON COLUMN Mobile.telecome is '통신사';
COMMENT ON COLUMN Mobile.userid is '아이디';
