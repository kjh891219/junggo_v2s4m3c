...
--입력
INSERT INTO SPORTS(SNO, category, nickname, passwd, deal_code, product_code, region, deal_way, hprice, purc_date, quantity, email, tel, title, content, userid)
...
--조회
SELECT sno, category, nickname, passwd, deal_code, product_code, region, deal_way, hprice, purc_date, quantity, email, tel, title, content
FROM SPORTS
ORDER BY sno desc;
...
-- 검색
-- title
 SELECT sno, category, title, hprice, region, deal_way, nickname, file1, file2, size2
  FROM sports
WHERE sno=6 AND title LIKE '%ㅋㅋㅋ%'
ORDER BY sno DESC;
 
-- content
 SELECT sno, category, title, hprice, region, deal_way, nickname, file1, file2, size2
  FROM sports
WHERE sno=1 AND content LIKE '%휴가%'
ORDER BY sno DESC;
 
-- 제목과 내용으로 검색
 SELECT sno, category, title, hprice, region, deal_way, nickname, file1, file2, size2
  FROM sports
WHERE sno=1 AND  title LIKE '%휴가%' OR content LIKE '%휴가%'
ORDER BY sno DESC;
...
--------------------------------------------------------------------
SELECT sno, category, title, hprice, region, deal_way, nickname, file1, file2, size2, wdate, r
FROM(
         SELECT sno, category, title, hprice, region, deal_way, nickname, file1, file2, size2, wdate,  rownum as r
         FROM(
                  SELECT sno, category, title, hprice, region, deal_way, nickname, file1, file2, size2 , wdate
                  FROM sports
