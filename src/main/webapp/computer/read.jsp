  ...
  <DIV class='content' style='width: 80%;'>
    <FORM name='frm' method='POST' action='./create.do' enctype="multipart/form-data">
      <div class='content_menu' style='width: 100%;'>
        <A href='./create.do?ctno=0&opentype=R'>등록</A>｜ <A href='./read.do?ctno=${computerVO.ctno }'>상세보기</A>｜ <A
          href='./create.do?ctno=${computerVO.ctno }&opentype=U'>수정</A>｜ <A href='./list.do'>목록</A>｜ <A
          href='./delete.do?ctno=${computerVO.ctno }'>삭제</A>｜ <A href="javascript:location.reload();">새로고침</A>
      </div>
      <fieldset>
        <label>글번호</label> <input type='text' name='ctno' id='ctno' value='${computerVO.ctno }'>
        <ul>
          <li><label for='deal_status'>*판매상태</label> <select name='deal_status' id='deal_status' class=" ">
          ...
          <li><div class="col-xs-5">
              <label for='quantity'>수량</label> <input type='number' name='quantity' id='quantity'
                value=${computerVO.quantity } class=" ">
            </div></li>
          <li><label class='label_1' for='userid'>등록자 ID</label> <input type='text' name='userid' id='userid'
            value='${computerVO.userid }' required="required"></li>
          <li><label class='label_1' for='email'>등록자 이메일</label> <input type='text' name='email' id='email'
            value='${computerVO.email }' required="required"> <label class='label_1' for='tel'>등록자 연락처</label> <input
            type='text' name='tel' id='tel' value='${computerVO.tel }' required="required"></li>
          ...
          <li><label for="file1MF" class="col-xs-2 col-lg-2 control-label">업로드 파일1:${computerVO.file1} </label>
            ${opentype == "U"? computerVO.file1:""}
            <div class="col-xs-10 col-lg-10">
              <input type="file" class="form-control" name='file1MF' id='file1MF' size='20'> <br>
              Preview(미리보기) 이미지 자동 생성됩니다.
            </div></li>
          <li><label for="file2MF" class="col-xs-2 col-lg-2 control-label">업로드 파일2:${computerVO.file2} </label>
            ${opentype == "U"? computerVO.file2:""}
            <div class="col-xs-10 col-lg-10">
              <input type="file" class="form-control" name='file2MF' id='file2MF' size='20'> <br>
              Preview(미리보기) 이미지 자동 생성됩니다.
            </div></li>
          <li><label for="file3MF" class="col-xs-2 col-lg-2 control-label">업로드 파일3:${computerVO.file3} </label>
            ${opentype == "U"? computerVO.file3:""}
            <div class="col-xs-10 col-lg-10">
              <input type="file" class="form-control" name='file3MF' id='file3MF' size='20'> <br>
              Preview(미리보기) 이미지 자동 생성됩니다.
            </div></li>
          <li><label for="file4MF" class="col-xs-2 col-lg-2 control-label">업로드 파일4:${computerVO.file4} </label>
            ${opentype == "U"? computerVO.file4:""}
            <div class="col-xs-10 col-lg-10">
              <input type="file" class="form-control" name='file4MF' id='file4MF' size='20'> <br>
              Preview(미리보기) 이미지 자동 생성됩니다.
            </div></li>
          <li><label for="file5MF" class="col-xs-2 col-lg-2 control-label">업로드 파일5:${computerVO.file5} </label>
            ${opentype == "U"? computerVO.file5:""}
            <div class="col-xs-10 col-lg-10">
          ...
          <li><label class='label' for='email'>등록자 이메일</label> <input type='text' name='email' id='email'
            value='${computerVO.email}' required="required"> <label class='label' for='tel'>등록자 연락처</label> <input
            type='text' name='tel' id='tel' value='${computerVO.tel}' required="required"></li>
