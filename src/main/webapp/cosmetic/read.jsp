...
<jsp:include page="/menu/left.jsp" flush='false' />
 ...
 <h1 style="padding-left: 0px; margin: 0; padding: 10px; float: left; display: block; font-size: 20px; line-height: 22px; font-weight: bold; color: #444;">
  <a href="${pageContext.request.contextPath}/cosmetic/list.do">화장품 게시판</a>
 </h1>
...
<span style="float: right; display: inline; font-weight: 400; letter-spacing: 0; color: #444; font-size: 12px;">${fn:substring(cosmeticVO.wdate, 0, 16) }</span>
<h1 style="font-size: 20px; font-weight: bold; color: #444; overflow: hidden; margin: 0; padding: 0 8px; line-height: 18px;">☞ ${cosmeticVO.title}</h1>
</div>
       ...
       <p class="photo_img" style="width:400px; height:400px; border:1px solid lightgray; border-bottom:none;">
         ...
         <div style="position: relative; float: right; width: 400px; margin-top: 50px; ">
          <strong style="font-size: 28px; font-weight: bold; color: #111; margin-top: 21px; text-align:center;">
           희망가격 :  <fmt:formatNumber value="${cosmeticVO.hprice }" pattern="#,###원"/>
          </strong>
       </div>        
                 ...
                 <li style="float: left; margin: 0; padding: 0;">
                          ...
                          <c:choose>
                            <c:when test="${fn:endsWith(file2, '.jpg')}">
                              <a href="./storage/${cosmeticVO.file2}">
                                 <IMG id='file2' class="gallery" src='./storage/${cosmeticVO.file2}' width="80" height="80" >
                              </a>
                            </c:when>
                        ...
                        <li style="float: left; margin: 0; padding: 0;">
                          ...
                          <c:choose>
                            <c:when test="${fn:endsWith(file4, '.jpg')}">
                              <a href="./storage/${cosmeticVO.file4}">
                                 <IMG id='file4' class="gallery" src='./storage/${cosmeticVO.file4}' width="80" height="80" >
                              </a>
                            </c:when>
                        ...
                        <li>
                          ...
                          <c:choose>
                            <c:when test="${fn:endsWith(file6, '.jpg')}">
                              <a href="./storage/${cosmeticVO.file6}">
                                 <IMG id='file6' class="gallery" src='./storage/${cosmeticVO.file6}' width="80" height="80" >
                              </a>
                            </c:when>
                        ...
                        <li>
                          ...
                          <c:choose>
                            <c:when test="${fn:endsWith(file8, '.jpg')}">
                              <a href="./storage/${cosmeticVO.file8}">
                                 <IMG id='file8' class="gallery" src='./storage/${cosmeticVO.file8}' width="80" height="80" >
                              </a>
                            </c:when>
                        ...
                        <li>
                          ...
                          <c:choose>
                            <c:when test="${fn:endsWith(file10, '.jpg')}">
                              <a href="./storage/${cosmeticVO.file10}">
                                 <IMG id='file10' class="gallery" src='./storage/${cosmeticVO.file10}' width="80" height="80" >
                              </a>
                            </c:when>
   ...
   <table style="width: 100%; border-top: 1px solid #cccccc; border-spacing: 0; display: table; border-collapse: separate; border-color: grey;'">
    ...
    <tr>
      ...
      <td style="padding: 14px 10px 14px 20px;  font: normal 12px '맑은 고딕'; color: #666; text-align: left; vertical-align: top; border-bottom: 1px solid #e5e5e5; letter-spacing: 0">
        <A href="javascript: profile(' ${cosmeticVO.userid}' ,' ${cosmeticVO.nickname}') ;" class='list_tag'  title='프로필'>${cosmeticVO.nickname}</A>
      </td>
    </tr>
    <tr>
      <th style="padding: 14px 10px 14px 20px; background: #f5f5f5; font: normal 12px '맑은 고딕'; color: #333; text-align: left; vertical-align: top; border-bottom: 1px solid #e5e5e5; letter-spacing: 0">
  ...
  <div style="position: relative; float: right; width: 580px; padding-right: 30px; margin-top: 50px;">
   ...
   <table style="width: 100%; border-top: 1px solid #cccccc; border-spacing: 0; display: table; border-collapse: separate; border-color: grey;'">
    ...
    <div style="text-align: center;">
      <c:if test ="${(cosmeticVO.userid ne userid)}">  
        <A href="javascript: send_wish( ' ${cosmeticVO.hprice}' ,' ${cosmeticVO.nickname}' , ' ${cosmeticVO.title}' ,' ${cosmeticVO.file1 }' )  ;" class='top_select'  title='위시리스트'>
          <IMG src='../images/favorite_love.png' alt="WishList"></A>
        <A href="javascript: msg_list(' ${cosmeticVO.userid}');" style="margin-left:50px" title='쪽지보내기'><IMG src='../images/Mail.png' alt="msgsend"></A>
    </c:if>
  </div> 
  </div>
</div>
<div style="clear: both;"></div>
 ...
 <div style="padding: 11px; border-top: 1px solid #CCC; border-bottom: 1px solid #CCC; background: #FCFCFC; display: block; margin-top: 50px; margin-bottom: 50px; display: block;">
  ...
  <button type='button' onclick="location.href='./list.do?cno=${cosmeticVO.cno}&col=${searchDTO.col}&word=${searchDTO.word}'">목록</button>
</div>
