...
<c:forEach var="livingVO" items="${list}">

  <TR>
    <TD class='td'>${livingVO.lno}</TD>
     <td class="td">
              <c:set var='file2' value="${fn:toLowerCase(livingVO.file2)}" />
              <c:choose>
                <c:when test="${fn:endsWith(file2, '.jpg')}">
                  <IMG id='file2' src='./storage/${livingVO.file1}'>
                </c:when>
                ...
                <c:otherwise>
                  <A href='${root}/download?dir=/living/storage&filename=${vo.file2}'>${vo.file2}</A> (${vo.size2Label})
                </c:otherwise>
              </c:choose>
            </td>
    ...
    <TD class='td'>${livingVO.cnt}</TD>
    <TD class='td'>${livingVO.wdate.substring(0,10) }</TD>
    <TD class='td'>
      <A href="./passwd.do?lno=${livingVO.lno}"><IMG src='./images/passwd.png' title='패스워드 변경'></A>
      <A href="./update.do?lno=${livingVO.lno}"><IMG src='./images/update.png' title='수정'></A>
      <A href="./delete.do?lno=${livingVO.lno}"><IMG src='./images/delete.png' title='삭제'></A>
    </TD>
  </TR>
</c:forEach>
 </tbody>
