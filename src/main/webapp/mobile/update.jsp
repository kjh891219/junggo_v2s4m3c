...
<FORM name='frm' method='POST' action='./update.do' class='form-inline' enctype="multipart/form-data">
   <input type='hidden' name='mno' id='mno' value='${mobileVO.mno}'>
<DIV class='content_form' style="margin-top:30px;">
     ...
     <div class="">
      ...
      <div class="row">
        ...
        <input type='text' name='title' id='title' required="required" value='${mobileVO.title} ' class="col-xs-9 col-lg-9">
      </div>
      <div>
        <label for='content' class='choice_e'>상세설명</label>
        <textarea rows="10" name="content" id="content" placeholder="내용을 입력하세요"  class="col-xs-9 col-lg-9">${mobileVO.content}</textarea>
      </div>
      <hr/>
        ...
        <label for="content" class='col-xs-2 col-lg-2 need'>업로드 파일1</label>
          ...
          <c:choose>
            <c:when test="${fn:endsWith(file2, '.jpg')}">
              <IMG src='./storage/${mobileVO.file1}'>
            </c:when>
      ...
      <div class='line_box' id='ul_box_1'></div>
        ...
        <input type='text' name='hprice' id='hprice'  required="required" value='${mobileVO.hprice}' class="col-xs-3 col-lg-3">원
        </div>
        <div class="row">       
        <label for='purc_date'  class='col-xs-2 col-lg-2 choice'>구입시기</label>
        <input type='text' name='purc_date' id='purc_date' value='${mobileVO.purc_date}' class="col-xs-3 col-lg-3"> 
        </div>
      <div class="row"> 
        <label for='quantity' class='col-xs-2 col-lg-2 choice'>수량</label>
        <input type='text' name='quantity' id='quantity' value='${mobileVO.quantity}' class="col-xs-3 col-lg-3">
      </div>
      ...
      <div class='inpo'>판매자 정보</div>
         <div class='line_box' id='ul_box_2'></div>
            <div class="row">
              ...
              <input type='text' name='nickname' id='nickname' value= '${mobileVO.nickname}' required="required" readonly="readonly" class="col-xs-3 col-lg-3"/>
              </div>
            <div class="row">
               <label for='tel'  class='col-xs-2 col-lg-2 need'>전화번호</label>
              <input type='text' name='tel' id='tel'  required="required" value='${mobileVO.tel }' class="col-xs-3 col-lg-3"> 
            </div>
            <div class="row">  
              <label for='email' class='col-xs-2 col-lg-2 need'>이메일</label>
              <input type='text' name='email' id='email'  required="required" value='${mobileVO.email }'  class="col-xs-3 col-lg-3">
            </div>
      ...
      <div class='text_r' >
        ...
        <button type="button" onclick="location.href='./list.do?mno=${mobileVO.mno}&col=${searchDTO.col}&word=${searchDTO.word}'" class="btn btn-danger btn-lg">취소</button>
      </div>
      </div>
</FORM>
</DIV>
